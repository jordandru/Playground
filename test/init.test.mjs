...
export const initTests = [
  {
    ...
    run() {
      withTempDir((root) => {
        ...
        assert.equal(typeof packageJson.scripts["infra:status"], "string");
        ...
        assert.equal(existsSync(path.join(targetDir, "scripts", "infra-status.mjs")), true);
        ...
        assert.equal(existsSync(path.join(targetDir, "src", "infra", "status.mjs")), true);
